...
# Load data
inputDataMat <- readRDS("processed_data/trajectory/20241223_input_osteogenicData_reshaped.Rds")
...
dir.create("processed_data/trajectory/20241223_bone_model_fits", recursive = TRUE, showWarnings = FALSE)
...
with_progress({
  p <- progressor(along = 160:length(genes))

  results <- foreach(
    gene_index = 160:length(genes),
    ...
    tryCatch({
      ...
      write.csv(
        weight,
        file = sprintf(
          "processed_data/trajectory/20241223_bone_model_fits//gene_%s_fit_weight.csv",
          genes[gene_index]
        )
      )
      saveRDS(
        fitModel,
        file = sprintf(
          "../tmp/20241223_traj_model_bone_run//gene_%s_fit_weight.Rds",
          genes[gene_index]
        )
      )
