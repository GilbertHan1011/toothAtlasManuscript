...
outdir <- "results/annotation/20241030_anno_step1/"
if (!dir.exists(outdir)) {
  dir.create(outdir, recursive = TRUE)
}

parameter_list = jsonlite::read_json("script/annotation/config/epithelium_parameter.json")
...
epithelium <- zellkonverter::readH5AD("processed_data/integrated_data/20241029_epithelium.h5ad")
#
varname <- read.csv("processed_data/attributeName/varName_epi.csv",row.names = 1) %>% unlist()
#
cellname <- read.csv("processed_data/attributeName/cellName_epi.csv",row.names = 1) %>% unlist()
rownames(epithelium) <- varname
colnames(epithelium) <- cellname
seurat <- Seurat::as.Seurat(epithelium,counts = "counts", data = "logcounts")

wtIntegrate_clusterint  = data.table::fread("processed_data/metadata/20241029_epi_leiden_metadata.csv",data.table = F)
...
# wtIntegrate_clusterint_clear <- subset(wtIntegrate_clusterint_clear, select = -Cell_ID...17)
# names(wtIntegrate_clusterint_clear)[1] <- 'Cell_ID'
...
p1 <- DimPlot(seurat,group.by = "leiden_clusters_0.05.x")
# DimPlot(seurat,group.by = "leiden_clusters_0.1.x")
p2 <- DimPlot(seurat,group.by = "leiden_clusters_0.175.x")
p3 <- DimPlot(seurat,group.by = "leiden_clusters_0.5.x")
p4 <- DimPlot(seurat,group.by = "leiden_clusters_1.x")
p5 <- DimPlot(seurat,group.by = "leiden_clusters_3.x")
p6 <- DimPlot(seurat,group.by = "leiden_clusters_5.x")
# p1 <-  DimPlot(seurat,group.by = "mix_level_2")
# p2 <- DimPlot(seurat,group.by = "leiden_clusters_0.05")
p1+p2+p3+p4+p5+p6

seurat$leiden_clusters_level_1 <- seurat$leiden_clusters_0.05.x
seurat$leiden_clusters_level_2 <- seurat$leiden_clusters_0.175.x
seurat$leiden_clusters_level_3 <- seurat$leiden_clusters_0.5.x
seurat$leiden_clusters_level_4 <- seurat$leiden_clusters_1.x
seurat$leiden_clusters_level_5 <- seurat$leiden_clusters_3.x
seurat$leiden_clusters_level_6 <- seurat$leiden_clusters_5.x
...
ggsave(paste0(outdir,"20241030_epi_annotation_level.png"),width = 8,height = 6)
...
data.table::fwrite(mrtree_input_labels,"processed_data/metadata//20241030_epi_anno_mrtree_input_label.csv",sep = ",")

saveRDS(seurat,"processed_data/integrated_data/20241030_epithelium.Rds")
